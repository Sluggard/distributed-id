...
public class NetClient {
    ...
    public NetClient(ServerNode serverNode, ZkClient zkClient, IdClient idClient) {
        ...
        try {
            ...
            bootstrap.group(bootGroup).channel(NioSocketChannel.class).handler(new ChannelInitializer<SocketChannel>() {
                @Override
            ...
            System.out.println("connect before：" + TimeUtil.now());
            ChannelFuture sync = bootstrap.connect(serverNode.getIp(), serverNode.getPort()).sync();
            System.out.println("connect after：" + TimeUtil.now());
    ...
    public NetClient sync(long timeMs) throws InterruptedException {
        this.wait(timeMs);
        if (this.defaultChannelInboundHandler.isClosed()) {
            throw new DistributedIdException("创建连接失败");
        }
        return this;
    }
    ...
    private ConnectListener getConnectionListener() {
        return connected -> {
            //建立连接后，向zk注册客户端信息
            if (connected) {
                System.out.println("getConnectionListener：" + TimeUtil.now());
